...
export interface Comment {
  ...
  content: string;
  timestamp: Date;
  isSystem?: boolean; // Nếu là thay đổi trạng thái
}
...
export class RequestCommentsComponent {
  @Input({ required: true }) requestId!: number;

  newComment = '';
  comments = signal<Comment[]>([
    {
      id: '1', authorName: 'System', authorRole: '', content: 'Yêu cầu được tạo', timestamp: new Date(Date.now() - 86400000), isSystem: true
    },
    {
      id: '2', authorName: 'user', authorRole: 'Nhân viên', content: 'Tôi đã tạo yêu cầu mua sắm laptop mới, sếp duyệt giúp.', timestamp: new Date(Date.now() - 80000000)
    },
    {
      id: '3', authorName: 'dept_lead', authorRole: 'Trưởng phòng', content: 'Giá laptop này hơi cao, bạn có check lại thị trường chưa?', timestamp: new Date(Date.now() - 3600000)
    }
  ]);
  ...
  sendComment(): void {
    ...
    this.comments.update(msgs => [...msgs, newMsg]);
    this.newComment = '';
  }
}
